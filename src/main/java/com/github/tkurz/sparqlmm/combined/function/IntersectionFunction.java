    ...
    public Value evaluate(ValueFactory valueFactory, Value... values) throws ValueExprEvaluationException {
        try {
            ...
            if(MediaFragments.temporalComparable(uris)) {
                temporal_fragment = uris[0].getMediaFragment().getTemporalFragment().getIntersection(uris[1].getMediaFragment().getTemporalFragment());
            }

            SpatialFragment spatial_fragment = null;
            if(MediaFragments.spatialComparable(uris)) {
                spatial_fragment = uris[0].getMediaFragment().getSpatialFragment().getIntersection(uris[1].getMediaFragment().getSpatialFragment());
            }
