 ...
 */
@FunctionDoc(title = "temporalContains", reference = FunctionDoc.Reference.temporal, description = "returns true if resource1.start <= resource2.start and resource1.end >= resource2.end, else false.")
        ...
        try {
            ...
            if (values.length == 3 && ((Literal)values[2]).booleanValue()) {
                return valueFactory.createLiteral(
                        uris[0].getMediaFragment().getTemporalFragment().contains(uris[1].getMediaFragment().getTemporalFragment()));
            } else {
                return valueFactory.createLiteral(
