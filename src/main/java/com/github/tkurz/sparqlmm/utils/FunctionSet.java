...
public class FunctionSet {
    ...
    public void toMD(File file) throws IOException {
        ...
        out.println("| :------------ |:------------|");
        for(Function function: functions) {
            if(function.type.equals(FunctionDoc.Type.extensionFunction.getUri())) out.printf("| %s:%s( *resource1*, *resource2* ) | %s |%n", Constants.ABBREVIATION, function.uri.stringValue().substring(Constants.NAMESPACE.length()), function.description);
        }
        out.println();
        out.println("## Aggregations");
        out.println();
        out.println("| Relation name | Description |");
        out.println("| :------------ |:------------|");
        for(Function function: functions) {
            if(function.type.equals(FunctionDoc.Type.extensionAggregate.getUri())) out.printf("| %s:%s( *resource1*, *resource2* ) | %s |%n", Constants.ABBREVIATION, function.uri.stringValue().substring(Constants.NAMESPACE.length()), function.description);
        }
